...
describe ResultsNotificationWorker do
  ...
  describe '#send_results_email' do
    ...
    let(:fun_question_id) { fun_question.id }
    let(:user_recipients_ids) { [user1.id, user2.id] }
    ...
    context 'when sending the results_email' do
      it 'sends an email with the correct data' do
        ...
        expect(UserEmailMailer).to receive(:results_email).with(user, time_period, an_instance_of(Array), fun_question_id, user_recipients_ids).and_call_original

        expect do
          worker.send(:send_results_email, user, time_period, fun_question_id, user_recipients_ids)
