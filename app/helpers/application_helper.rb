module ApplicationHelper
  ...
  def check_period?
    start_day = ENV.fetch('START_WEEK_DAY', 'tuesday').strip.capitalize
    end_day = ENV.fetch('DAY_TO_SEND_INVITES', 'friday').strip.capitalize
    current_day = Date.current.wday
    current_day >= Date::DAYNAMES.index(start_day) && current_day < Date::DAYNAMES.index(end_day)
  end

  def check_in_period?
    invite_day_name = ENV.fetch('DAY_TO_SEND_INVITES', 'friday').strip.capitalize
    final_day_name = ENV.fetch('DAY_TO_SEND_FINAL_REMINDER', 'monday').strip.capitalize
