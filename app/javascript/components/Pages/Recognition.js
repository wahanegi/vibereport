...
const Recognition = ({data, setData, saveDataToDb, steps, service, draft}) => {
  ...
  const editHandling = (e) => {
    e.preventDefault();
    setIsDraft(false);
    const editObj = data.user_shoutouts.find(
        item => item.id === Number(e.target.attributes.id.value));
                ...
                <div
                  ...
                  <span className="expand-link"
                        onMouseEnter={(e) =>
                            e.currentTarget.querySelector('.trash').
                                classList.
                                add('d-none') ||
                            e.currentTarget.querySelector('.trashRed').
                                classList.
                                remove('d-none')}
                        onMouseLeave={(e) =>
                            e.currentTarget.querySelector('.trash').
                  ...
                  >
                                <img src={trash} alt="trash"
                                     className="trash img-fluid pointer"
                                     onClick={trashHandling}/>
                                <img id={'trashRed' + shoutOut.id}
                                     src={trashRed} alt="trash"
                                     className="trashRed d-none img-fluid pointer"
                                     onClick={trashHandling}/>
                            </span>
                </div>
              </li>
  ...
  return (
      ...
      >
        <div className="container-fluid pt-1 pt-md-0">
          ...
          {!numShoutOuts &&
              <div className="mb-6">
                ...
                <div className="shoutout-container position-relative">
                  <ShoutoutButton data={data} setData={setData}
                                  isDraft={isDraft} saveDataToDb={saveDataToDb}
                                  steps={steps}
                                  draft={isDraft}
                                  handleSaveDraft={handleSaveDraft}
                                  isMove={true}/>
                </div>
                <p className="gray-600 mt-1 fw-bold">Click to give a
                  Shoutout!</p>
              </div>
          }
          ...
          {isModal &&
              <ShoutoutDelete onClose={onClose} data={data} setData={setData}
                              idShoutout={idShoutout}/>}
          <BlockLowerBtns skipHandling={skipHandling}
                          nextHandling={nextHandling} isNext={!!numShoutOuts}/>
        </div>
      </Layout>
  );
};
