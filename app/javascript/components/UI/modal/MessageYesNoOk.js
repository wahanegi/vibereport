...
const MessageYesNoOk = ({ content, onClick, onClose, isNoYesIsNotOk = true } ) => {
    return <div className = 'placement-modal-message'>
        ...
        <div className={`mt-${isNoYesIsNotOk ? 6 : 0} d-flex justify-content-around`}>
            {isNoYesIsNotOk && <button className='btn btn-modal c1 back' onClick={ onClose }>No, go back</button>}
            {isNoYesIsNotOk && <button className='btn btn-modal c1 delete' onClick={ onClick }>Yes, delete</button>}
